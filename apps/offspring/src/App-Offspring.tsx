...
import { listOffspringGroups, listOffspringByGroup, type UiOffspringGroup, type UiOffspring } from "@bhq/api";
...
function OffspringPage() {
  ...
  React.useEffect(() => {
    listOffspringGroups()
      .then(g => { setGroups(g); if (g[0]) setSelected(g[0]); })
  ...
  React.useEffect(() => {
    if (!selected) return;
    setKids([]);
    listOffspringByGroup(selected.id).then(setKids).catch(console.error);
  }, [selected?.id]);
